...
import { HOME, SHOP } from '../../constants/path'
...
export default function ProductDetail() {
    ...
    useEffect(async () => {
        let res = await productServices.getDetail(slug)
        if (res) setDetail(res)
    }, [slug])
    // useEffect(()=>{

    // },[indexSlider])
    // data-flickity="{&quot;draggable&quot;: false, &quot;fade&quot;: true}" id="productSlider"
    ...
    const setIndex = (i, ev) => {
        ...
        let selected = document.querySelector('.is-nav-selected')
        if (selected) document.querySelector('.is-nav-selected').classList.remove('is-nav-selected')
        let a = ev.target
        a.classList.toggle('is-nav-selected')
        flkty?.current.flkty.select(i)
        // flkty?.select(i)
        // console.log(`flkty._events`, flkty._events)
    ...
    return (
        <>
            <Breadcrumb>
                ...
                <BreadcrumbItem to={SHOP}>Women's Shoes</BreadcrumbItem>
                <BreadcrumbItem to="#">Leather Sneaker</BreadcrumbItem>
            </Breadcrumb>
            {/* PRODUCT */}
            <section>
                            ...
                            <div className="row">
                                <div className="col-12 col-md-6">
                                    ...
                                    <div className="card">
                                        ...
                                        {/* Slider */}
                                        <Flickity options={flickityOptions} ref={flkty} classname="mb-4" >
                                            {/* Item */}
                                            {
                                                detail.images.map((o, i) => (
                                                    <a href={o?.base_url} key={i} style={{ height: '550px', width: '540px', overflow: 'hidden' }}  >
                                                        <img src={o?.base_url} alt="..." style={{ height: "100%", width: "100%", objectFit: "cover" }} />
                                    ...
                                    {/* Slider */}
                                    <Flickity options={flickityOptionsSmall} className="flickity-nav mx-n2 mb-10 mb-md-0"  >
                                        {/* Item */}
                                        {
                                            detail.images.map((o, i) => (
                                                <div className="col-12 px-2" style={{ maxWidth: '113px' }} onClick={(ev) => { setIndex(i, ev) }}>
                                                    {/* Image */}
                                                    <div className="embed-responsive embed-responsive-1by1 bg-cover tab_item " style={{ backgroundImage: `url(${o?.small_url})` }} />
                                                </div>
                                            ))
                                        }
                                    </Flickity>
                                ...
                                <div className="col-12 col-md-6 pl-lg-10">
                                    ...
                                    {/* Price */}
                                    <div className="mb-7">
                                        <span className="font-size-lg font-weight-bold text-gray-350 text-decoration-line-through"> {detail?.price} VNĐ </span>
                                        <span className="ml-1 font-size-h5 font-weight-bolder text-primary"> {detail?.real_price}VNĐ </span>
                                    ...
                                    <form>
                                        <div className="form-group">
                                            {/* Label */}
                                            <p className="mb-5">
                                                Color: <strong id="colorCaption">{detail?.color} </strong>
                                            </p>
                                            {/* Radio */}
                                            <div className="mb-8 ml-n1">
                                                {
                                                    detail.configurable_products.map((o, i) => (
                                                        i === 0 ? (
                                                            <div className="custom-control custom-control-inline custom-control-img">
                                                                <input type="radio" className="custom-control-input" id={`${i}`} name="imgRadio" data-toggle="form-caption" data-target="#colorCaption" defaultChecked />
                                                                <label className="custom-control-label" htmlFor={`${i}`}>
                                                                    <span className="embed-responsive embed-responsive-1by1 bg-cover" style={{ backgroundImage: `url(${o?.images[0]?.small_url})` }} />
                                                                </label>
                                                            </div>)
                                                            : (<div className="custom-control custom-control-inline custom-control-img">
                                                                <input type="radio" className="custom-control-input" id={`${i}`} name="imgRadio" data-toggle="form-caption" data-target="#colorCaption" />
                                                                <label className="custom-control-label" htmlFor={`${i}`}>
                                                                    <span className="embed-responsive embed-responsive-1by1 bg-cover" style={{ backgroundImage: `url(${o?.images[0]?.small_url})` }} />
                                                                </label>
                                                            </div>)))
                                                ...
                                                {/* <div className="custom-control custom-control-inline custom-control-img">
                                                    <input type="radio" className="custom-control-input" id="imgRadioOne" name="imgRadio" data-toggle="form-caption" data-target="#colorCaption" defaultValue="White" />
